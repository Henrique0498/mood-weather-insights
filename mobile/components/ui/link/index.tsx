...
const Link = React.forwardRef<React.ComponentRef<typeof UILink>, ILinkProps>(
  function Link(
    { className, href, replace, children, onPress, ...props },
    ref
  ) {
    ...
    const handlePress = React.useCallback(
      (e: any) => {
        // Call any provided onPress first
        onPress?.(e);
        if (e?.defaultPrevented) return;

        if (href) {
          // Always navigate internally using expo-router
          if (replace) router.replace(href as any);
          else router.push(href as any);
        }
      },
      [href, replace, router, onPress]
    );
